...
+ (nonnull RollbarMutableConfig *)mutableConfigWithAccessToken:
  ...
  RollbarMutableConfig *config =
      [[RollbarMutableConfig alloc] initWithAccessToken:token environment:nil];

  return config;
}

+ (nonnull RollbarMutableConfig *)
    mutableConfigWithAccessToken:(nonnull NSString *)token
                     environment:(nonnull NSString *)env {
  ...
  RollbarMutableConfig *config =
      [[RollbarMutableConfig alloc] initWithAccessToken:token environment:env];
...
- (instancetype)initWithAccessToken:(nullable NSString *)token
                        environment:(nullable NSString *)env {
...
- (void)setModifyRollbarData:
    (RollbarData *_Nonnull (^)(RollbarData *_Nonnull))modifyRollbarData {
...
- (void)setDeveloperOptions:(RollbarDeveloperOptions *)developerOptions {
  [self setDataTransferObject:[developerOptions mutableCopy]
                       forKey:DFK_DEVELOPER_OPTIONS];
}
...
- (void)setLoggingOptions:(RollbarLoggingOptions *)loggingOptions {
  [self setDataTransferObject:[loggingOptions mutableCopy]
                       forKey:DFK_LOGGING_OPTIONS];
}
