...
- (void)testBasicDTOInitializationWithJSONString {
  NSString *jsonString = @"{\"access_token\":\"ACCESS_TOKEN\", "
                         @"\"data\":{\"environment\":\"ENV\"}}";
  NSString *jsonPayload = @"{\"access_token\":\"ACCESS_TOKEN\"}";
  NSString *jsonData = @"{\"environment\":\"ENV\"}";

  RollbarPayload *payloadAtOnce =
      [[RollbarPayload alloc] initWithJSONString:jsonString];
  XCTAssertNotNil(payloadAtOnce, @"Payload instance");
  XCTAssertTrue([payloadAtOnce.accessToken isEqualToString:@"ACCESS_TOKEN"],
                @"Access token field [%@] of payload: %@.",
                payloadAtOnce.accessToken,
                [payloadAtOnce serializeToJSONString]);
  XCTAssertNotNil(payloadAtOnce.data, @"Data field of payload: %@.",
                  [payloadAtOnce serializeToJSONString]);
  XCTAssertTrue([payloadAtOnce.data.environment isEqualToString:@"ENV"],
                @"Environment field [%@] of payload: %@.",
                payloadAtOnce.data.environment,
                [payloadAtOnce serializeToJSONString]);

  RollbarPayload *payload =
      [[RollbarPayload alloc] initWithJSONString:jsonPayload];
  RollbarData *payloadData = [[RollbarData alloc] initWithJSONString:jsonData];
  payload.data = payloadData;
  XCTAssertTrue([[payloadAtOnce serializeToJSONString]
                    isEqualToString:[payload serializeToJSONString]],
  ...
  XCTAssertTrue(![payload hasSameDefinedPropertiesAs:payloadData],
                @"RollbarPayload and RollbarData DTOs do not have same defined "
                @"properties");
...
- (void)testRollbarProxyDTO {
  ...
  RollbarProxy *dto = [[RollbarProxy alloc] initWithEnabled:proxyEnabled
                                                   proxyUrl:proxyUrl
                                                  proxyPort:proxyPort];
  XCTAssertTrue(dto.enabled == proxyEnabled, @"Enabled.");
  XCTAssertTrue(dto.proxyPort == proxyPort, @"Enabled.");
  XCTAssertTrue(dto.proxyUrl == proxyUrl, @"Enabled.");
}

- (void)testRollbarScrubbingOptionsDTO {
  RollbarMutableScrubbingOptions *dto = [[RollbarMutableScrubbingOptions alloc]
      initWithScrubFields:@[ @"field1", @"field2" ]];
  XCTAssertTrue(dto.enabled, @"Enabled by default");
  XCTAssertTrue(dto.scrubFields.count == 2, @"Has some scrub fields");
  XCTAssertTrue(dto.safeListFields.count == 0, @"Has NO whitelist fields");

  dto.safeListFields = [@[ @"tf1", @"tf2", @"tf3" ] mutableCopy];
  XCTAssertTrue(dto.safeListFields.count == 3, @"Has some whitelist fields");

  dto.enabled = NO;
  XCTAssertTrue(!dto.enabled, @"Expected to be disabled");
}

- (void)testRollbarServerConfigDTO {
  RollbarMutableServerConfig *dto =
      [[RollbarMutableServerConfig alloc] initWithHost:@"HOST"
                                                  root:@"ROOT"
                                                branch:@"BRANCH"
                                           codeVersion:@"1.2.3"];
  XCTAssertTrue(NSOrderedSame == [dto.host compare:@"HOST"], @"Proper host");
  XCTAssertTrue(NSOrderedSame == [dto.root compare:@"ROOT"], @"Proper root");
  ...
  dto.host = @"h1";
  XCTAssertTrue(NSOrderedSame == [dto.host compare:@"h1"], @"Proper new host");
  dto.root = @"r1";
  XCTAssertTrue(NSOrderedSame == [dto.root compare:@"r1"], @"Proper new root");
  dto.branch = @"b1";
  ...
  XCTAssertNil(branchValue, @"Uninitialized branch");
...
- (void)testRollbarPersonDTO {
  RollbarMutablePerson *dto =
      [[RollbarMutablePerson alloc] initWithID:@"ID"
                                      username:@"USERNAME"
                                         email:@"EMAIL"];
  XCTAssertTrue(NSOrderedSame == [dto.ID compare:@"ID"], @"Proper ID");
  XCTAssertTrue(NSOrderedSame == [dto.username compare:@"USERNAME"],
                @"Proper username");
  XCTAssertTrue(NSOrderedSame == [dto.email compare:@"EMAIL"], @"Proper email");

  dto.ID = @"ID1";
  XCTAssertTrue(NSOrderedSame == [dto.ID compare:@"ID1"], @"Proper ID");
  ...
  XCTAssertTrue(NSOrderedSame == [dto.ID compare:@"ID007"], @"Proper ID");
  XCTAssertNil(dto.username, @"Proper default username");
  XCTAssertNil(dto.email, @"Proper default email");
}

- (void)testRollbarModuleDTO {
  RollbarMutableModule *dto =
      [[RollbarMutableModule alloc] initWithName:@"ModuleName"
                                         version:@"v1.2.3"];
  XCTAssertTrue([dto.name isEqualToString:@"ModuleName"], @"Proper name");
  XCTAssertTrue([dto.version isEqualToString:@"v1.2.3"], @"Proper version");
  ...
  XCTAssertNil(dto.version, @"Proper version");
}

- (void)testRollbarTelemetryOptionsDTO {
  RollbarScrubbingOptions *scrubber = [[RollbarScrubbingOptions alloc]
      initWithEnabled:YES
          scrubFields:@[ @"one", @"two" ]
       safeListFields:@[ @"two", @"three", @"four" ]];
  RollbarTelemetryOptions *dto =
      [[RollbarTelemetryOptions alloc] initWithEnabled:YES
                                            captureLog:YES
                                   captureConnectivity:YES
                                    viewInputsScrubber:scrubber];
  XCTAssertTrue(dto.enabled, @"Proper enabled");
  XCTAssertTrue(dto.captureLog, @"Proper capture log");
  XCTAssertTrue(dto.captureConnectivity, @"Proper capture connectivity");
  ...
  XCTAssertTrue(dto.viewInputsScrubber.enabled,
                @"Proper view inputs scrubber enabled");
  XCTAssertTrue(dto.viewInputsScrubber.scrubFields.count ==
                    [RollbarScrubbingOptions new].scrubFields.count,
                @"Proper view inputs scrubber scrub fields count");
  XCTAssertTrue(dto.viewInputsScrubber.safeListFields.count ==
                    [RollbarScrubbingOptions new].safeListFields.count,
                @"Proper view inputs scrubber white list fields count");
}

- (void)testRollbarLoggingOptionsDTO {
  RollbarMutableLoggingOptions *dto = [[RollbarMutableLoggingOptions alloc]
             initWithLogLevel:RollbarLevel_Error
  ...
  XCTAssertTrue(dto.logLevel == RollbarLevel_Error, @"Proper log level");
  XCTAssertTrue(dto.crashLevel == RollbarLevel_Info, @"Proper crash level");
  ...
  XCTAssertNil(dto.codeVersion, @"Proper default code version");
  XCTAssertTrue([dto.framework isEqualToString:@"macos"] ||
                    [dto.framework isEqualToString:@"ios"],
                @"Proper default framework");
  XCTAssertNil(dto.requestId, @"Proper request ID");
}
...
- (void)testRollbarConfigDTO {
  ...
  [rc setServerHost:@"SERVERHOST"
               root:@"SERVERROOT"
             branch:@"SERVERBRANCH"
        codeVersion:@"SERVERCODEVERSION"];
  [rc setNotifierName:@"NOTIFIERNAME" version:@"NOTIFIERVERSION"];

  RollbarMutableConfig *rcClone = [[RollbarMutableConfig alloc]
      initWithJSONString:[rc serializeToJSONString]];

  //    id scrubList = rc.scrubFields;
  //    id scrubListClone = rcClone.scrubFields;

  XCTAssertTrue([rc isEqual:rcClone], @"Two DTOs are expected to be equal");
  //    XCTAssertTrue([[rc serializeToJSONString] isEqualToString:[rcClone
  ...
  rcClone = [[RollbarMutableConfig alloc]
      initWithJSONString:[rc serializeToJSONString]];
  rcClone.httpProxy.proxyUrl = @"SOME_OTHER_ONE";
  XCTAssertTrue(![rc isEqual:rcClone],
                @"Two DTOs are NOT expected to be equal");
  XCTAssertTrue(
      [rcClone isEqual:[[RollbarConfig alloc]
                           initWithJSONString:[rcClone serializeToJSONString]]],
...
- (void)testRollbarRequestDTO {
  ...
  NSDictionary *getParams =
      @{@"GET_PARAM_1" : @"GETPARAM1", @"GET_PARAM_2" : @"GETPARAM2"};
...
- (void)testRollbarCallStackFrameContextDTO {
  ...
  RollbarCallStackFrameContext *dto =
      [[RollbarCallStackFrameContext alloc] initWithPreCodeLines:pre
                                                   postCodeLines:post];
...
- (void)testRollbarCallStackFrameDTO {
  ...
  NSArray<NSString *> *pre = @[ @"CODE_PR1", @"CODE_PR2" ];
  NSArray<NSString *> *post = nil;
  RollbarCallStackFrameContext *codeContext =
      [[RollbarCallStackFrameContext alloc] initWithPreCodeLines:pre
                                                   postCodeLines:post];
  ...
  RollbarCallStackFrame *dto =
      [[RollbarCallStackFrame alloc] initWithFileName:filename];
  XCTAssertNotNil(dto);
  ...
  XCTAssertEqual(dto.context.preCodeLines.count,
                 codeContext.preCodeLines.count);
...
- (void)testRollbarTraceDTO {
  ...
  RollbarCallStackFrame *frameDto =
      [[RollbarCallStackFrame alloc] initWithFileName:filename];
  ...
  RollbarTrace *dto =
      [[RollbarTrace alloc] initWithRollbarException:exceptionDto
...
// if/when we do allow a trace side-by-side with the crash report uncomment this
// test:
//-(void)testRollbarBodyDTO {
//
//    RollbarBody *body = [[RollbarBody alloc]
//    initWithCrashReport:CRASH_REPORT_PLCRASH_SYMBOLICATED];
//
...
- (void)testRollbarTelemetryEventDTO_properBodyBasedOnType {
  ...
  XCTAssertNotNil(event.body);
  XCTAssertTrue(
      [event.body isKindOfClass:[RollbarTelemetryNavigationBody class]]);
  ...
  XCTAssertNotNil(event.body);
  XCTAssertTrue(
      [event.body isKindOfClass:[RollbarTelemetryConnectivityBody class]]);
...
- (void)testRollbarTelemetryEventDTO_Log {
  ...
  RollbarTelemetryBody *body =
      [[RollbarTelemetryLogBody alloc] initWithMessage:logMessage
                                             extraData:extra];
...
- (void)testRollbarTelemetryEventDTO_View {
  ...
  RollbarTelemetryBody *body =
      [[RollbarTelemetryViewBody alloc] initWithElement:viewElement
                                              extraData:extra];
...
- (void)testRollbarTelemetryEventDTO_Error {
  ...
  RollbarTelemetryBody *body =
      [[RollbarTelemetryErrorBody alloc] initWithMessage:logMessage
                                               extraData:extra];
...
- (void)testRollbarTelemetryEventDTO_Navigation {
  ...
  NSString *to = @"TO";
  RollbarTelemetryBody *body =
      [[RollbarTelemetryNavigationBody alloc] initWithFromLocation:from
                                                        toLocation:to
                                                         extraData:extra];
...
- (void)testRollbarTelemetryEventDTO_Network {
  ...
  RollbarTelemetryBody *body =
      [[RollbarTelemetryNetworkBody alloc] initWithMethod:method
                                                      url:url
  ...
  XCTAssertEqual(((RollbarTelemetryNetworkBody *)event.body).statusCode,
                 statusCode);
}
...
- (void)testRollbarTelemetryEventDTO_Connectivity {
  ...
  RollbarTelemetryBody *body =
      [[RollbarTelemetryConnectivityBody alloc] initWithStatus:status
                                                     extraData:extra];
  ...
  XCTAssertEqual(((RollbarTelemetryConnectivityBody *)event.body).status,
                 status);
}
...
- (void)testRollbarTelemetryEventDTO_Manual {
  ...
  RollbarTelemetryBody *body =
      [[RollbarTelemetryManualBody alloc] initWithDictionary:extra];
